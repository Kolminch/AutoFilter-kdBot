...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        buttons = [[
                    ...
                    InlineKeyboardButton('ꜱʜᴀʀᴇ ʙᴏᴛ', url='https://telegram.me/share/url?url=https://bit.ly/3E19G7l')
                ],[
...
async def auto_filter(client, msg, spoll=False):
    ...
    if imdb and imdb.get('poster'):
        try:
            if settings['auto_delete']:
                hehe = await message.reply_photo(photo=imdb.get('poster'), caption=cap[:1024] + "\n\n<b>‣ Tʜɪs Mᴇssᴀɢᴇ Wɪʟʟ ʙᴇ Aᴜᴛᴏ-Dᴇʟᴇᴛᴇᴅ Aғᴛᴇʀ 05 Mɪɴᴜᴛᴇs.</b>", reply_markup=InlineKeyboardMarkup(btn))
...
async def advantage_spell_check(msg):
    ...
    g_s = await search_google(query)
    g_s += await search_google(msg.text)
    gs_parsed = []
    if not g_s:
        btn = [[
